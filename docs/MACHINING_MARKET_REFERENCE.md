...
Because the existing `market-bootstrap` workflow combines schema migration with public-history bootstrap, Phase 2B adds `.github/workflows/market-db-migrate.yml` as a separate safety boundary. It is `workflow_dispatch` only, has no schedule, grants `contents: read`, and runs the fixed sequence `npm ci`, `npm run check`, `npm test`, `npm run db:migrate`, followed by a machine-readable success assertion for `DATABASE_MIGRATED`. Production database variables (`NODE_ENV=production`, `STORAGE_PROVIDER=postgres`, `DATABASE_SSL=true`, `REQUIRE_DURABLE_STORAGE=1`, and the existing `DATABASE_URL` secret) are scoped only to the migration step; they are not present at job scope and cannot leak into dependency installation, code checks, or deterministic tests.

The workflow does not call `production:bootstrap`, `production:daily`, `production:weekly`, `weekly:send`, any backfill command, Gmail, or mail configuration. It is safe to run repeatedly because the existing migration contract uses idempotent `CREATE TABLE IF NOT EXISTS` and `CREATE INDEX IF NOT EXISTS` statements; the deterministic Postgres migration test runs the migration twice and requires the same statement count and successful state on both runs. The workflow-contract test explicitly verifies job/test env isolation and the absence of forbidden operational commands. This hotfix task does not dispatch the workflow or execute the migration against production.
