...
Render Free is the verified public dashboard/web host at [`https://raw-material-market-dashboard-1.onrender.com`](https://raw-material-market-dashboard-1.onrender.com). Its health checks reported `WEB_READY`, `DATABASE_READY`, PostgreSQL durable storage, 36 ms observed database latency, `BOOTSTRAP_COMPLETE`, `persistedRecordCount: 11351`, `SEND_OK`, 14/14 usable indicators and `WEEKLY_REPORT_READY`. When configured with `STORAGE_PROVIDER=postgres` and owner-provided `DATABASE_URL`, it reads the same Postgres public history and does not require a persistent disk. It never owns scheduled Gmail SMTP delivery. `MAIL_CONFIGURATION_REQUIRED` on Render is expected because Render is web/dashboard hosting only; Gmail credentials remain in GitHub Actions secrets. No paid resource activation was performed.
