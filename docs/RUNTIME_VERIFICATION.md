...
| Actions weekly | `market-weekly.yml` has Monday schedule／manual dispatch, migration, quality gate, JSON／HTML／XLSX, Graph delivery and duplicate guard |
| Graph safety | Personal `consumers` account only; delegated `Mail.Send`, `MAIL_TEST_MODE`／`MAIL_TEST_TO`, dry-run, token/error redaction, bounded Graph retry and no silent SMTP fallback |
...
A controlled production simulation uses only a synthetic public-safe fixture and a temporary root／fake pool. It verifies migration contract, storage provider selection, bootstrap／daily／weekly lifecycle, quality gate, dry-run, duplicate guard, health and public export. Graph/OAuth tests use mocked fetch responses only. No real Neon connection, Microsoft OAuth exchange, Graph send, Gmail connection, Actions activation, paid resource or deployment is used.
