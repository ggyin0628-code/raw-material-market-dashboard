...
Use only a temporary absolute `PRODUCTION_STORAGE_ROOT` and synthetic/public-safe records. Required sequence is storage check → public bootstrap or deterministic backfill → daily snapshot → completed-week analytics／quality gate → JSON／HTML／XLSX → Graph-provider dry-run → duplicate guard → `/health/weekly` → public backup. Never use real recipient addresses, owner secrets or generated private runtime reports in Git.

A separate bounded live public smoke may report current source availability, material counts, history and FX; provider failures are operational observations, not offline code gaps. Real Microsoft OAuth exchange, Graph sendMail receipt and scheduler enablement are intentionally excluded from this repository verification.

## Production simulation result

Historical filesystem production simulation completed with synthetic public-safe records on `feat/weekly-market-intelligence-production-v1`: unconfigured storage returned `STORAGE_CONFIGURATION_REQUIRED`／not ready; configured absolute temporary root returned `DURABLE_CONFIGURED`; bootstrap returned `BOOTSTRAP_COMPLETE` with 45 persisted records; daily snapshot returned `OK` with 15 records／15 inserted; weekly generated JSON／HTML／XLSX and returned `SEND_WITH_WARNINGS` plus mail `DRY_RUN`／`sent: false`; duplicate guard returned `DUPLICATE_PREVENTED`; `/health/weekly` returned HTTP 200 with no path or secret leak; public-only backup manifest was created. No SMTP socket, OAuth token exchange or real email was used. Exact safe output is retained outside the repository at `/home/ubuntu/raw-material-production-simulation.md`.

The historical filesystem production gate recorded `31 passed／0 failed`; the current pre-remediation zero-cost deterministic gate was `38 passed／0 failed`; the bootstrap-remediation gate was `43 passed／0 failed`; the current Outlook Graph mail-provider gate is **53 passed／0 failed**. `npm run check`, `npm run build` and `npm audit --omit=dev` pass with 0 vulnerabilities. Production storage, Microsoft app registration/refresh token, approved sender／recipients, TEST_RECIPIENT receipt and scheduler activation remain `EXTERNAL_CONFIGURATION_REQUIRED` by design.

## Zero-Cost Runtime V1 verification

The bootstrap-remediation branch `fix/bootstrap-performance-v1` and certified tag `bootstrap-performance-certified-v1` remain the performance baseline, based on main `8390a0234fb5d18e28e100ee1ff40750b6b0d95e`. This mail change is implemented on feature branch `feat/outlook-graph-mail-v1` and is pending final promotion. Its current deterministic suite is **53 passed／0 failed**; `npm run check`, `npm run build`, `npm audit --omit=dev`, YAML checks and the clean-tree check pass. The Graph provider/OAuth tests do not require a real Microsoft account or Neon account.
...
| Actions weekly | `market-weekly.yml` has Monday schedule／manual dispatch, migration, quality gate, JSON／HTML／XLSX, Gmail SMTP delivery and duplicate guard |
| Mail safety | Gmail SMTP remains the active production provider; `WEEKLY_MAIL_TEST_MODE`／`MAIL_TEST_TO`, dry-run, secret redaction, duplicate guard and no credential exposure |
| CLI output | `production:weekly` returns concise safe summary with reporting week, quality, provider/state, counts, artifact names and duration; no full report/history dump |
| Health | `/health/weekly` readiness states and redaction; database URL／OAuth tokens／recipient lists absent |
| Product boundary | Public external market intelligence only; no company or supplier private data |

A controlled production simulation uses only a synthetic public-safe fixture and a temporary root／fake pool. It verifies migration contract, storage provider selection, bootstrap／daily／weekly lifecycle, quality gate, dry-run, duplicate guard, health and public export. Any historical Graph/OAuth tests use mocked fetch responses only. The verified production checkpoint separately confirms the Render web deployment and owner-confirmed Gmail SMTP test; this documentation update performs no new Neon connection, Gmail connection, Actions activation or paid-resource operation.

The final GitHub-only fresh clone was created from the promoted `main` at the presentation SHA and ran `npm ci`, `npm run check`, `npm test`, `npm run build`, `npm audit --omit=dev`, unconfigured Postgres fail-closed checks, workflow source checks and safe production simulation. It remained clean. The verified Render checkpoint at `https://raw-material-market-dashboard-1.onrender.com` reports `WEB_READY`, `DATABASE_READY`, `BOOTSTRAP_COMPLETE`, `persistedRecordCount: 11351`, `SEND_OK`, 14/14 usable indicators and `WEEKLY_REPORT_READY`; observed database latency was 36 ms. The owner-confirmed Gmail SMTP `TEST_SENT` state includes successful HTML email and XLSX attachment receipt. The next owner action is schedule enablement by setting `PRODUCTION_SCHEDULES_ENABLED=1` when ready; the manual Gmail test is complete and must not be repeated.
