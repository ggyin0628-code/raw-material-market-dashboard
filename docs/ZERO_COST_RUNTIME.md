...
Neon-compatible PostgreSQL
    ↓
Microsoft Graph `/me/sendMail` using delegated personal account
```

Render Free remains optional web/dashboard hosting. It serves the dashboard and reads the same PostgreSQL-backed public history when `STORAGE_PROVIDER=postgres`; it has no scheduled mail responsibility and does not use its local filesystem for durable data.

## Mail provider boundary

Production weekly delivery uses `MAIL_PROVIDER=outlook_graph`, `MICROSOFT_TENANT=consumers`, delegated `Mail.Send`, `MICROSOFT_CLIENT_ID` and secret-managed `MICROSOFT_REFRESH_TOKEN` for the personal account `ggyin0628@hotmail.com`. The Graph adapter refreshes access tokens in memory and sends HTML plus XLSX via JSON `fileAttachment`. The one-time `npm run microsoft:oauth` helper is owner-controlled, does not send mail, refuses repository output and never prints tokens. The previous Gmail SMTP adapter is retained only for explicit `MAIL_PROVIDER=smtp` compatibility; production weekly no longer injects Gmail SMTP configuration. Company Microsoft 365 accounts, application permissions and paid mail resources are out of scope.
...
A missing database URL, failed migration, unavailable database, invalid public payload, transaction rollback, quality-blocked report, missing Graph configuration, refresh-token revocation, Graph `401`/`403`/`429`/`5xx`, bounded timeout, attachment failure or duplicate weekly send produces an explicit state. Partial public-source failures may produce `SEND_WITH_WARNINGS` when the report contract permits; materially unusable data produces `SEND_BLOCKED` and no mail-provider attempt. Graph errors are redacted and never fall back silently to SMTP. The owner uses `workflow_dispatch` for manual recovery rather than creating artificial commits or silently fabricating market data.
