...
| U.S. BLS PPI Steel Pipe and Tube, Stainless Steel, FRED `WPU10170674` | FRED exposes a downloadable graph CSV, monthly frequency, index Dec 2010=100, source U.S. BLS, and history from 2010-12 through 2026-07.[2] | `GLOBAL_IMPORT_REFERENCE` | Retained as external stainless-market context only with `participatesInScoring=false`; product scope is stainless pipe/tube, not Taiwan stainless sheet, so it cannot affect `materialPressure`. |
...
Each normalized source retains `sourceId`, `sourceName`, `geographicScope`, `marketScope`, `marketRole`, `pricingBasis`, `currency`, `unit`, `frequency`, `status`, `observationDate`, `lastObservationDate`, `fetchedAt`, endpoint, `participatesInScoring`, `scoringReason` and note. The four allowed roles are `TAIWAN_DOMESTIC`, `GLOBAL_IMPORT_REFERENCE`, `GLOBAL_INPUT_PROXY` and `STRUCTURAL`.

The Phase 3A model keeps six major dimensions and minimum evidence of 3. Multiple eligible sources can contribute to one dimension. Material pressure may therefore contain Taiwan basic-metals PPI, international HRC, aluminum, cold-rolled and copper references, and nickel upstream pressure. The stainless pipe/tube series remains visible in provenance but is excluded before evidence counts, comparison windows and scores because its product scope does not match stainless sheet. The response retains role-level provenance and explains the distinction; no company-specific domestic/import weight is assumed.
