...
const { observationAgeDays, weeklyMaxObservationAgeDays } = require("../marketData/freshness");

const FRESH_STATUSES = new Set(["LIVE", "FALLBACK"]);
const CURRENT_OBSERVATION_STATUSES = new Set(["LIVE", "FALLBACK", "STALE"]);
...
function latestCurrentRecord(records, predicate = () => true) {
  return latestRecord((records || []).map(normalizedRecord).filter((record) => CURRENT_OBSERVATION_STATUSES.has(record.status) && finite(record.marketPrice)), predicate);
}
...
function buildSignal(indicator) {
  ...
  if (indicator.latestValidObservation && indicator.currentObservationFreshness && !indicator.currentObservationFreshness.eligible) {
    return {
      signal: "DATA_QUALITY_WARNING",
      reasonCodes: [indicator.currentObservationFreshness.status === "EXPIRED" ? "CURRENT_OBSERVATION_EXPIRED" : "CURRENT_OBSERVATION_NOT_FRESH"],
      reason: `目前可取得的最後觀測距離報告週末 ${indicator.currentObservationFreshness.ageDays == null ? "未知" : `${indicator.currentObservationFreshness.ageDays.toFixed(0)} 天`}，未達週報 freshness policy，不列入 headline。`,
    };
  }
...
function buildIndicatorAnalytics(material, records, period, options = {}) {
  ...
  const latestValidObservation = latestCurrentRecord(materialRecords, (record) => record.date <= period.end);
  const maxObservationAgeDays = Number.isFinite(options.maxObservationAgeDays) ? options.maxObservationAgeDays : weeklyMaxObservationAgeDays(options.env || process.env);
  const currentObservationFreshness = latestValidObservation ? {
    status: latestValidObservation.status,
    ageDays: observationAgeDays(latestValidObservation.date, period.end),
    eligible: observationAgeDays(latestValidObservation.date, period.end) != null && observationAgeDays(latestValidObservation.date, period.end) <= maxObservationAgeDays,
  } : { status: "NO_DATA", ageDays: null, eligible: false };
  ...
  const currentForComparison = currentObservationFreshness.eligible ? latestValidObservation : null;
  const weekly = calculateChange(currentForComparison, fresh, "weekly", period);
  const fourWeek = calculateChange(currentForComparison, fresh, "fourWeek", period);
  const threeMonth = calculateChange(currentForComparison, fresh, "threeMonth", period);
  const ytd = calculateChange(currentForComparison, fresh, "ytd", period);
  const fiftyTwoWeek = calculateChange(currentForComparison, fresh, "fiftyTwoWeek", period);
  const indicator = {
    ...
    currentObservationFreshness,
    headlineEligible: currentObservationFreshness.eligible && finite(weekly.value),
...
function sourceCoverage(records, period, options = {}) {
  ...
  const counts = { LIVE: 0, FALLBACK: 0, STALE: 0, EXPIRED: 0, NO_DATA: 0, API_ERROR: 0 };
  const maxAgeDays = Number.isFinite(options.maxObservationAgeDays) ? options.maxObservationAgeDays : weeklyMaxObservationAgeDays(options.env || process.env);
  let freshnessEligibleCount = 0;
  let latestObservationDate = null;
  for (const record of latestByMaterial.values()) {
    counts[record.status] = (counts[record.status] || 0) + 1;
    const ageDays = observationAgeDays(record.date, period.end);
    if (CURRENT_OBSERVATION_STATUSES.has(record.status) && ageDays != null && ageDays <= maxAgeDays) freshnessEligibleCount += 1;
    if (record.date && (!latestObservationDate || record.date > latestObservationDate)) latestObservationDate = record.date;
  }
  return {
    counts,
    observedIndicators: latestByMaterial.size,
    totalIndicators: materials.length,
    freshnessEligibleCount,
    headlineEligibleCount: freshnessEligibleCount,
    latestObservationDate,
    coveragePct: materials.length ? freshnessEligibleCount / materials.length * 100 : 0,
  };
}

function buildWeeklyAnalytics(records, reportingWeek, options = {}) {
  const period = typeof reportingWeek === "string" ? parseReportingWeek(reportingWeek) : reportingWeek;
  const indicators = materials.map((material) => buildIndicatorAnalytics(material, records, period, options));
  const fx = buildFxAnalytics(records, period);
  const coverage = sourceCoverage(records, period, options);
  const qualityWarnings = indicators.filter((indicator) => ["DATA_QUALITY_WARNING", "DATA_INSUFFICIENT"].includes(indicator.signal));
  const headlineEligibleIndicators = indicators.filter((indicator) => indicator.headlineEligible);
  const risers = headlineEligibleIndicators.sort((a, b) => b.weeklyChangePct - a.weeklyChangePct).slice(0, 3);
  const decliners = headlineEligibleIndicators.sort((a, b) => a.weeklyChangePct - b.weeklyChangePct).slice(0, 3);
  ...
  return {
    ...
    summary: {
      ...
      freshnessEligibleCount: indicators.filter((indicator) => indicator.currentObservationFreshness?.eligible).length,
      headlineEligibleCount: headlineEligibleIndicators.length,
      expiredObservationCount: indicators.filter((indicator) => indicator.currentObservationFreshness?.status === "EXPIRED").length,
    },
  };
}
