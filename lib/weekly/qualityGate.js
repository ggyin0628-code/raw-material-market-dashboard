...
function evaluateWeeklyQuality(reportLike = {}, options = {}) {
  ...
  const statuses = { LIVE: 0, FALLBACK: 0, STALE: 0, EXPIRED: 0, NO_DATA: 0, API_ERROR: 0, ...(reportLike.qualitySummary?.statuses || {}) };
  ...
  const expiredCount = Number(statuses.EXPIRED || 0);
  const fallbackCount = Number(statuses.FALLBACK || 0);
  const freshnessEligibleCount = Number(reportLike.qualitySummary?.freshnessEligibleCount ?? reportLike.sourceCoverage?.freshnessEligibleCount ?? 0);
  const freshnessCoveragePct = trackedIndicatorCount ? freshnessEligibleCount / trackedIndicatorCount * 100 : 0;
  ...
  const minimumFreshnessCoveragePct = Number.isFinite(options.minimumFreshnessCoveragePct) ? options.minimumFreshnessCoveragePct : 50;
  const materiallyUnusable = !trackedIndicatorCount || !usableIndicatorCount || usableIndicatorCount / trackedIndicatorCount < 0.5;
  const freshnessInsufficient = !freshnessEligibleCount || freshnessCoveragePct < minimumFreshnessCoveragePct;
  ...
  if (expiredCount) warningReasons.push("EXPIRED_PRESENT");
  if (fallbackCount) warningReasons.push("FALLBACK_PRESENT");
  if (freshnessInsufficient) integrityReasons.push("OBSERVATION_FRESHNESS_INSUFFICIENT");
  ...
  return {
    ...
    freshnessCoveragePct,
    minimumFreshnessCoveragePct,
