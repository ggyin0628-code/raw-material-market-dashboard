...
const DEFAULT_REPORT_DIR = path.resolve(__dirname, "../../data/weekly-reports");
...
async function writeWeeklyReportFiles(report, outDir = DEFAULT_REPORT_DIR) {
  await fs.mkdir(outDir, { recursive: true });
  const safeWeek = report.reportingWeek.replace(/[^0-9W-]/g, "_");
  const jsonPath = path.join(outDir, `weekly-market-intelligence-${safeWeek}.json`);
  const htmlPath = path.join(outDir, `weekly-market-intelligence-${safeWeek}.html`);
  const xlsxPath = path.join(outDir, `weekly-market-intelligence-${safeWeek}.xlsx`);
...
async function loadAndBuildWeeklyReport(options = {}) {
  const records = options.records || await listSnapshots({ filePath: options.filePath, env: options.env });
