...
const { addDays, previousCompletedWeek, parseReportingWeek } = require("./weekUtils");
...
// The 52-week target is 364 days before period.end and its current comparison
// policy permits a record up to 420 days before that target. Keep this bounded
// range conservative so sparse histories retain the existing comparison rules.
const MAX_ANALYTICS_LOOKBACK_DAYS = 364 + 420;
...
function getWeeklySnapshotRange(reportingWeek) {
  const week = typeof reportingWeek === "string" ? parseReportingWeek(reportingWeek) : reportingWeek;
  if (!week || !week.end || !/^\d{4}-\d{2}-\d{2}$/.test(String(week.end))) throw new Error("週報 snapshot range 缺少有效的 reporting week end");
  const ytdStart = `${week.end.slice(0, 4)}-01-01`;
  const comparisonStart = addDays(week.end, -MAX_ANALYTICS_LOOKBACK_DAYS);
  return { from: comparisonStart < ytdStart ? comparisonStart : ytdStart, to: week.end, reportingWeek: week.reportingWeek };
}

async function loadAndBuildWeeklyReport(options = {}) {
  const week = options.reportingWeek ? parseReportingWeek(options.reportingWeek) : previousCompletedWeek(options.generatedAt || new Date());
  const range = getWeeklySnapshotRange(week);
  const records = options.records || await listSnapshots({ filePath: options.filePath, env: options.env, storageConfig: options.storageConfig, pool: options.pool, from: range.from, to: range.to });
  return buildWeeklyReport({ records, reportingWeek: week.reportingWeek, generatedAt: options.generatedAt });
}
