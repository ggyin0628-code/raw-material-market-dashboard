...
async function run(command, argv = process.argv.slice(3), env = process.env) {
  ...
  const error = new Error("未知命令。可用：daily:snapshot、weekly:backfill、weekly:report、weekly:preview、weekly:send、production:storage-check、production:status、production:bootstrap、production:daily、production:weekly、production:backup");
  error.statusCode = 2;
  throw error;
}

function commandExitCode(result) {
  const states = [result?.state, result?.storage?.state, result?.storage?.errorCode].filter(Boolean);
  if (states.some((state) => ["FAILED", "SEND_BLOCKED", "STORAGE_CONFIGURATION_REQUIRED", "BOOTSTRAP_REPORT_BLOCKED"].includes(state))) return 2;
  return 0;
}

function errorExitCode(error) {
  if (["FAILED", "SEND_BLOCKED", "STORAGE_CONFIGURATION_REQUIRED", "BOOTSTRAP_REPORT_BLOCKED", "SNAPSHOT_STORE_INVALID", "JOB_STATE_INVALID", "REPORT_METADATA_INVALID", "DELIVERY_LEDGER_INVALID"].includes(error?.code)) return 2;
