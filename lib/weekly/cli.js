...
function safeArtifactNames(artifacts = {}) {
  return Object.fromEntries(Object.entries(artifacts).filter(([key]) => key !== "metadata").map(([key, value]) => [key, typeof value === "string" && value ? path.basename(value) : null]));
}

function summarizeProductionWeekly(result, env, durationMs) {
  const quality = result?.qualityGate || {};
  const mailConfig = require("./mailService").readMailConfig(env);
  const mail = result?.mail || {};
  return {
    reportingWeek: result?.reportingWeek,
    qualityGate: {
      state: quality.state,
      trackedIndicatorCount: quality.trackedIndicatorCount,
      usableIndicatorCount: quality.usableIndicatorCount,
      materialUsabilityPct: quality.materialUsabilityPct,
    },
    mail: {
      provider: mail.provider || mailConfig.provider,
      testMode: mail.testMode ?? mailConfig.testMode,
      state: mail.state || "NOT_REQUESTED",
      recipientCount: Number.isFinite(mail.recipientCount) ? mail.recipientCount : mailConfig.envelopeRecipients.length,
      attachmentCount: Number.isFinite(mail.attachmentCount) ? mail.attachmentCount : (result?.artifacts?.xlsxPath ? 1 : 0),
      errorCode: mail.errorCode || null,
      error: mail.error || null,
    },
    artifacts: safeArtifactNames(result?.artifacts),
...
async function run(command, argv = process.argv.slice(3), env = process.env) {
  ...
  if (command === "production:weekly") {
    const startedAt = Date.now();
    const result = await runProductionWeekly({ reportingWeek: options.week, filePath: options.file, outDir: options.out_dir, ledgerPath: options.ledger, env, dryRun: Boolean(options.dryRun), send: options.send !== false, allowResend: Boolean(options.allowResend) });
    return summarizeProductionWeekly(result, env, Date.now() - startedAt);
  }
