...
const DEFAULT_SNAPSHOT_FILE = path.join(PROJECT_ROOT, "data", "market-snapshots", "snapshots.json");
...
function getSnapshotFilePath(env = process.env) {
  const configured = String(env.MARKET_SNAPSHOT_FILE || "").trim();
  if (!configured) return DEFAULT_SNAPSHOT_FILE;
...
async function upsertSnapshots(records, options = {}) {
  ...
  const filePath = options.filePath || getSnapshotFilePath(options.env);
...
async function listSnapshots(options = {}) {
  const store = await readStore(options.filePath || getSnapshotFilePath(options.env));
