...
async function sendWeeklyEmail({ report, html, xlsxBuffer, dryRun = false, env = process.env, ledgerPath, allowResend = false, storageConfig, pool, smtpSender = smtpSend, graphSender = graphSend } = {}) {
  ...
  const operation = async () => {
    ...
    if (!config.enabled) return { state: "FAILED", provider: config.provider, reportingWeek: report.reportingWeek, configValid: configCheck.valid, error: "MAIL_ENABLED 未啟用，已 fail closed", sent: false };
    if (!configCheck.valid) return { state: "FAILED", provider: config.provider, reportingWeek: report.reportingWeek, configValid: false, configErrors: configCheck.errors, sent: false };
    try {
      if (config.provider === "outlook_graph") {
      ...
      return { state: "FAILED", provider: config.provider, reportingWeek: report.reportingWeek, configValid: true, error: redacted, sent: false, testMode: config.testMode };
    }
  };
