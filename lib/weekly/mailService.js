...
async function writeLedger(ledger, filePath = DEFAULT_DELIVERY_LEDGER) {
  return writeFileAtomic(filePath, JSON.stringify(ledger, null, 2), { encoding: "utf8" });
}
...
function isTransientMailError(error) {
  ...
  if (["ETIMEDOUT", "ECONNRESET", "ECONNREFUSED", "EAI_AGAIN", "ENETUNREACH"].includes(error.code)) return true;
...
async function sendWeeklyEmail({ report, html, xlsxBuffer, dryRun = false, env = process.env, ledgerPath, allowResend = false, storageConfig, smtpSender = smtpSend } = {}) {
  ...
  const targetLedger = ledgerPath || getStorageConfig(env).deliveryLedgerFile;
  const configForState = storageConfig || getStorageConfig(env);
  const operation = async () => {
    await updateJobState("weeklyMail", { state: "RUNNING", lastAttemptedAt: new Date().toISOString(), reportingWeek: report.reportingWeek }, { config: configForState }).catch(() => {});
    ...
    if (dryRun) {
      const result = { state: "DRY_RUN", reportingWeek: report.reportingWeek, configValid: configCheck.valid, configErrors: configCheck.errors, recipientCount: config.envelopeRecipients.length, attachmentCount: attachments.length, testMode: config.testMode, sent: false };
      ...
      await updateJobState("weeklyMail", { state: "DRY_RUN", lastCompletedAt: new Date().toISOString(), reportingWeek: report.reportingWeek, sent: false }, { config: configForState }).catch(() => {});
      return result;
    }
    if (["SENT", "TEST_SENT"].includes(prior?.state) && !resendApproved) {
      ...
      await writeLedger(next, targetLedger);
      await updateJobState("weeklyMail", { state: "DUPLICATE_PREVENTED", lastCompletedAt: duplicateAt, reportingWeek: report.reportingWeek, sent: false }, { config: configForState }).catch(() => {});
      return { state: "DUPLICATE_PREVENTED", reportingWeek: report.reportingWeek, configValid: configCheck.valid, recipientCount: config.envelopeRecipients.length, sent: false };
    }
    ...
    try {
      ...
      await writeLedger(next, targetLedger);
      await updateJobState("weeklyMail", { state, lastSuccessfulAt: new Date().toISOString(), reportingWeek: report.reportingWeek, sent: true, testMode: config.testMode }, { config: configForState }).catch(() => {});
      return { state, reportingWeek: report.reportingWeek, configValid: true, recipientCount: config.envelopeRecipients.length, attachmentCount: attachments.length, sent: true, testMode: config.testMode };
    } catch (error) {
      const redacted = safeError(error);
      const next = { weeks: { ...(ledger.weeks || {}), [report.reportingWeek]: { state: "FAILED", failedAt: new Date().toISOString(), error: redacted, testMode: config.testMode, recipientCount: config.envelopeRecipients.length } } };
      await writeLedger(next, targetLedger).catch(() => {});
      await updateJobState("weeklyMail", { state: "FAILED", lastFailedAt: new Date().toISOString(), reportingWeek: report.reportingWeek, lastError: redacted, sent: false }, { config: configForState }).catch(() => {});
      return { state: "FAILED", reportingWeek: report.reportingWeek, configValid: true, error: redacted, sent: false, testMode: config.testMode };
    }
  };
