...
async function sendWeeklyEmail({ report, html, xlsxBuffer, dryRun = false, env = process.env, ledgerPath, allowResend = false, storageConfig, pool, smtpSender = smtpSend } = {}) {
  ...
  const operation = async () => {
    ...
    try {
      ...
      await updateJobState("weeklyMail", { state: "FAILED", lastFailedAt: new Date().toISOString(), reportingWeek: report.reportingWeek, lastError: redacted, sent: false }, { config: configForState, env, pool }).catch(() => {});
