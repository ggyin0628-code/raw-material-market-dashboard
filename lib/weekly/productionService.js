...
const { collectAndPersistDailySnapshot, dailyReadinessState } = require("./dailySnapshotService");
...
async function runProductionWeekly(options = {}) {
  ...
  try {
    ...
    if (options.send !== false && qualityGate.readyForDelivery) {
      ...
      const send = options.sendWeeklyEmail || sendWeeklyEmail;
      mail = await send({
...
async function readProductionStatus(options = {}) {
  ...
  const databaseState = config.provider === "postgres"
    ...
    : "DATABASE_NOT_USED";
  const dailyState = dailyReadinessState(publicJobs.dailySnapshot);
