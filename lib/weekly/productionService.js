...
const { migratePostgres } = require("./postgresAdapter");
...
async function runProductionBootstrap(options = {}) {
  ...
  await updateJobState("productionBootstrap", { state: "RUNNING", lastAttemptedAt: startedAt }, { ...storageOptions(options, config) });
  try {
    const backfill = options.backfill
      ? await options.backfill({ period: options.period || "3y", filePath: options.filePath, env, storageConfig: config, pool: options.pool, collectedAt: startedAt })
      : await backfillPublicHistory({ period: options.period || "3y", filePath: options.filePath, env, storageConfig: config, pool: options.pool, collectedAt: startedAt });
    const recordCount = (await listSnapshots({ filePath: options.filePath, env, storageConfig: config, pool: options.pool })).length;
    ...
    await updateJobState("productionBootstrap", { state: result.state, lastSuccessfulAt: new Date().toISOString(), persistedRecordCount: recordCount, reportingWeek: weekly.reportingWeek, qualityGate: weekly.qualityGate }, { ...storageOptions(options, config) });
    return result;
  } catch (error) {
    await updateJobState("productionBootstrap", { state: "FAILED", lastFailedAt: new Date().toISOString(), lastError: safeError(error) }, { ...storageOptions(options, config) }).catch(() => {});
    throw error;
  }
}
