...
function calculateProcessTime({ input, workload, profile = null }) {
  ...
  if (input.cutting.enabled) {
    formulaTrace.push(traceEntry("totalCuttingRunMinutes", "totalCutLengthMm ÷ cuttingSpeedMmPerMin", { totalCutLengthMm: round(cutLengthMm), cuttingSpeedMmPerMin: profile.cutting.cuttingSpeedMmPerMin }, "mm ÷ (mm/min) = min", cutting.runMinutes, "min", profile));
    formulaTrace.push(traceEntry("totalPierceMinutes", "totalPierceCount × pierceTimeSecondsEach ÷ 60", { totalPierceCount: pierceCount, pierceTimeSecondsEach: profile.cutting.pierceTimeSecondsEach }, "each × s/each ÷ 60 = min", cutting.pierceMinutes, "min", profile));
    formulaTrace.push(traceEntry("totalCuttingSetupMinutes", "setupMinutesPerBatch × batchCount", { setupMinutesPerBatch: profile.cutting.setupMinutesPerBatch, batchCount }, "min/batch × batch = min", cutting.setupMinutes, "min", profile));
  ...
  if (input.bending.enabled) {
    formulaTrace.push(traceEntry("totalBendingRunMinutes", "totalBendCount × secondsPerBend ÷ 60", { totalBendCount: bendCount, secondsPerBend: profile.bending.secondsPerBend }, "each × s/each ÷ 60 = min", bending.runMinutes, "min", profile));
    formulaTrace.push(traceEntry("totalBendingSetupMinutes", "setupMinutesPerBatch × batchCount", { setupMinutesPerBatch: profile.bending.setupMinutesPerBatch, batchCount }, "min/batch × batch = min", bending.setupMinutes, "min", profile));
  ...
  if (input.welding.enabled) {
    formulaTrace.push(traceEntry("totalWeldingRunMinutes", "totalWeldLengthMm ÷ weldingSpeedMmPerMin", { totalWeldLengthMm: round(weldLengthMm), weldingSpeedMmPerMin: profile.welding.weldingSpeedMmPerMin }, "mm ÷ (mm/min) = min", welding.runMinutes, "min", profile));
    formulaTrace.push(traceEntry("totalWeldingSetupMinutes", "setupMinutesPerBatch × batchCount", { setupMinutesPerBatch: profile.welding.setupMinutesPerBatch, batchCount }, "min/batch × batch = min", welding.setupMinutes, "min", profile));
