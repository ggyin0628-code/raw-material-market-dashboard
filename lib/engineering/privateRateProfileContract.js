...
const METADATA_KEYS = new Set(["source", "owner", "approvalStatus", "note"]);
...
function validatePrivateRateProfile(profile) {
  ...
  if (profile.approvalStatus !== undefined) validateText(profile.approvalStatus, "rateProfile.approvalStatus", errors, { allowNull: true });
  validateText(profile.currency, "rateProfile.currency", errors);
  if (profile.metadata !== undefined) {
    if (requireObject(profile.metadata, "rateProfile.metadata", errors)) {
      validateKeys(profile.metadata, METADATA_KEYS, "rateProfile.metadata", errors);
      for (const key of METADATA_KEYS) if (Object.prototype.hasOwnProperty.call(profile.metadata, key)) validateText(profile.metadata[key], `rateProfile.metadata.${key}`, errors);
    }
  }
...
function getPrivateRateProfileSchema() {
  return {
    ...
    metadata: {
      ...
      required: false,
      fields: {
        source: { type: "string", output: "never returned" },
        owner: { type: "string", output: "never returned" },
        approvalStatus: { type: "string", output: "never returned" },
        note: { type: "string", output: "never returned" },
      },
    },
    material: {
