...
const { getPublicPriceReferences, validatePublicPriceReferences } = require("../publicProcessPriceContract");
...
function buildComponent(definition = {}) {
  ...
  const excluded = normalized.filter((observation) => observation.provenance.participatesInScoring === false);
  const usable = normalized.filter((observation) => observation.provenance.participatesInScoring !== false && sourceStatusIsUsable(observation.status) && observation.value !== null);
  if (!usable.length) {
    const sourceProvenance = normalized.map((observation) => observation.provenance);
    const dataQuality = normalized.length ? qualityFromStatuses(normalized.filter((observation) => observation.provenance.participatesInScoring !== false).map((observation) => observation.status)) : (definition.status || "NO_DATA");
    return {
      ...
      explanation: [definition.noDataReason, ...excluded.map((observation) => `排除計分：${observation.label}；${observation.provenance.scoringReason}`)].filter(Boolean),
    };
  }
  ...
  const withProvenance = (reason) => ({
    ...
    sourceProvenance: normalized.map((observation) => observation.provenance),
    explanation: [reason, ...excluded.map((observation) => `排除計分：${observation.label}；${observation.provenance.scoringReason}`)].filter(Boolean),
    comparisonWindows,
  ...
  return {
    ...
    sourceProvenance: normalized.map((item) => item.provenance),
    explanation: [...explanation, ...excluded.map((item) => `排除計分：${item.label}；${item.provenance.scoringReason}`)],
    ...
    observedValues: usable.map((item) => ({
      ...
      participatesInScoring: item.provenance.participatesInScoring,
      scoringReason: item.provenance.scoringReason,
...
function buildSheetMetalReference(input = {}) {
  ...
  const scoringSources = COMPONENT_IDS.flatMap((id) => (components[id].observedValues || []).map((item) => ({ marketRole: item.marketRole })));
  ...
  const reference = {
    ...
    scoringSourceRoleSummary: summarizeMarketRoles(scoringSources),
    publicPriceReferences: input.publicPriceReferences || getPublicPriceReferences({ process: "SHEET_METAL" }),
    explanation: [
      ...explanation,
  ...
  const priceErrors = validatePublicPriceReferences(reference.publicPriceReferences);
  if (errors.length || priceErrors.length) throw new Error(`sheet-metal reference contract invalid: ${[...errors, ...priceErrors].join("; ")}`);
  return reference;
}
