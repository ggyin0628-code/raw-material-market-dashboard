...
const GLOBAL_SOURCE_CATALOG = Object.freeze({
  coldRolledSteel: {
    ...
    scoringReason: "Product scope matches cold-rolled steel sheet and strip as an international market reference; not Taiwan domestic or CIF pricing.",
  ...
  stainlessSteel: {
    ...
    scoringReason: "Product scope mismatch: stainless pipe/tube is retained only as external stainless-market context, not sheet-metal price evidence.",
    updateFrequency: "Monthly",
    frequency: "monthly",
  ...
  nickel: {
    ...
    currency: "USD",
    participatesInScoring: true,
    scoringReason: "Upstream nickel input proxy for stainless pressure; no finished-sheet or alloy conversion is asserted.",
    updateFrequency: "Monthly",
    frequency: "monthly",
    unit: "USD/metric ton",
...
function globalSourceRecord(catalog, status, history, fetchedAt, note = catalog.note) {
  ...
  return { ...catalog, participatesInScoring: catalog.participatesInScoring !== false, scoringReason: catalog.scoringReason || "", status, lastObservationDate, observationDate: lastObservationDate, fetchedAt, note };
}
...
function unavailableSource(sourceId, sourceName, note) {
  return {
    ...
    participatesInScoring: false,
    scoringReason: "No accepted Taiwan domestic public price series; retained as an explicit gap only.",
    fetchedAt: null,
    note,
  };
}
...
function classifyRawMarketResult(result, marketRole) {
  ...
  return {
    ...
    source: {
      ...
      participatesInScoring: result.source.participatesInScoring !== false,
      scoringReason: result.source.scoringReason || "",
...
function classifyDomesticResult(result, { marketRole = "TAIWAN_DOMESTIC", marketScope = "Taiwan domestic public indicator", pricingBasis = "Official Taiwan public indicator; not a supplier quotation", currency = null } = {}) {
  return {
    ...
    source: {
      ...
      participatesInScoring: result.source.participatesInScoring !== false && marketRole !== "STRUCTURAL",
      scoringReason: result.source.scoringReason || (marketRole === "STRUCTURAL" ? "Structural tariff metadata has no generic comparable price and must not generate momentum." : ""),
      observationDate: result.source.observationDate || result.source.lastObservationDate || null,
    },
  };
}
