...
function nearestFxRate(fxByDate, date) {
  ...
  return dates.length ? fxByDate.get(dates[dates.length - 1]) : null;
}

function addPriceCalculations(material, priceRows, fxRows, source) {
...
function buildDecision(material, rows, period, insufficient) {
  ...
  const rangeText = `${current.toLocaleString("zh-TW", { maximumFractionDigits: 4 })}`;
  const fullRangeText = `${low.toLocaleString("zh-TW", { maximumFractionDigits: 4 })} ~ ${high.toLocaleString("zh-TW", { maximumFractionDigits: 4 })}`;

  let signal = "建議觀望";
  let action = "先觀察下一輪供應商報價";
  let risk = insufficient ? `${period.label}歷史資料不足，趨勢判斷需保守。` : "公開期貨行情不等於供應商現貨報價。";

  if (position >= 0.85 || latestChange >= 0.05) {
    signal = "高風險";
    action = "暫緩追價，先確認合約價與替代料";
  } else if (position >= 0.7 || latestChange >= 0.02) {
    signal = "成本上升";
    action = "確認供應商報價有效期，避免一次性大量買高";
  } else if (latestChange <= -0.04) {
    signal = "成本下降";
    action = "追蹤供應商是否同步反映行情下跌";
  } else if (position <= 0.3 || latestChange <= -0.02) {
    signal = "可議價";
    action = "要求供應商依行情下修報價";
  } else if ((high - low) / current > 0.25) {
    signal = "建議分批採購";
    action = "分批下單，降低短期波動風險";
  } else {
    signal = "穩定";
    action = "維持正常採購節奏";
  }
...
async function getFxRows(period) {
  try {
    ...
    await logMarket("export_fx_failed", { error: error.message });
    const wrapped = new Error(`匯率抓不到：${error.message}`);
...
async function getHistoricalRows(material, period, fxRows) {
  try {
    ...
    return {
      material,
      rows: addPriceCalculations(material, result.rows, fxRows, result.source),
      insufficient: isDataInsufficient(result.rows, period),
...
function buildWorkbook(dataset, period) {
  ...
  const yearlyRows = dataset.flatMap((item) => {
    ...
    return rows.map((row, index) => ({
      ...
      low: row.low,
      changePercent: row.changePercent,
      yearOverYear: index > 0 && rows[index - 1].average ? (row.average - rows[index - 1].average) / rows[index - 1].average : null,
    }));
  });
  addRows(yearly, [
    { header: "原物料", key: "materialName" },
  ...
  addRows(decision, [
    ...
    { header: "近 3 年高低區間", key: "threeYearRange" },
    { header: "目前位階", key: "position" },
    { header: "採購訊號", key: "signal" },
    { header: "建議動作", key: "action" },
    { header: "風險說明", key: "risk" },
  ], allDecisions);

  const missing = dataset.filter((item) => item.error || item.insufficient);
  if (missing.length) {
    const note = workbook.addWorksheet("資料狀態");
    addRows(note, [
      { header: "原物料", key: "material" },
      ...
      status: item.error ? "匯出失敗" : "資料不足",
      message: item.error || `${period.label}資料不足，Excel 已依可取得資料計算。`,
    })));
  }
...
async function createHistoricalWorkbook({ symbol, period: periodInput, all = false }) {
  ...
  const hasRows = dataset.some((item) => item.rows.length);
  if (!hasRows) {
  ...
  return {
    buffer,
    filename: all ? `all-raw-materials-${periodInput}.xlsx` : `${selectedMaterials[0].id}-${periodInput}.xlsx`,
  };
}
...
async function createHistoryPayload({ symbol, period: periodInput }) {
  ...
  const monthly = analyzePeriod(dataset.rows, (row) => row.date.slice(0, 7));
  const yearly = analyzePeriod(dataset.rows, (row) => row.date.slice(0, 4));
  const decision = buildDecision(material, dataset.rows, period, dataset.insufficient);
