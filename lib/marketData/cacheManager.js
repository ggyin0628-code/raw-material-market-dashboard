...
const { allowSeedFallback, getSnapshotDataAsOf, isProduction, shouldServeStale } = require("./freshness");
...
async function getStaleCache(reason, options = {}) {
  ...
  for (const candidate of candidates) {
    ...
    const snapshot = canonicalizeSnapshot(entry.snapshot);
    if (isStaleUsable({ ...entry, snapshot }, now, env)) {
      if (candidate.tier === "local") memoryCache = entry;
      await logMarket("stale_trigger", { tier: candidate.tier, reason, dataAsOf: getSnapshotDataAsOf(snapshot) });
      return markSnapshotStale(snapshot, reason, now, env);
    }
    if (getSnapshotDataAsOf(snapshot)) {
      await logMarket("expired_trigger", { tier: candidate.tier, reason, dataAsOf: getSnapshotDataAsOf(snapshot) });
      return markSnapshotExpired(snapshot, reason, now);
    }
  }
