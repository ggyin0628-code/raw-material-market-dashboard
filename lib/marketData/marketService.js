...
function buildSnapshot(fx, rows) {
  ...
  const staleRows = rows.filter((row) => row.status === MARKET_STATES.STALE).length;
  const errorRows = rows.filter((row) => row.status === MARKET_STATES.API_ERROR).length;
  const state = okRows > 0 ? MARKET_STATES.OK : fallbackRows > 0 ? MARKET_STATES.FALLBACK : staleRows > 0 ? MARKET_STATES.STALE : MARKET_STATES.API_ERROR;
...
async function hydrateFailedRowsWithStale(snapshot) {
  ...
  const stale = await getStaleCache("部分即時行情來源失敗");
  ...
  await logMarket("stale_row_hydration", {
    hydratedRows,
  ...
  const staleFx = snapshot.fx?.status === MARKET_STATES.API_ERROR && stale.fx?.rate
    ...
    : snapshot.fx;
  const hydratedSnapshot = buildSnapshot(staleFx, rows);
  hydratedSnapshot.disclaimer = `${hydratedSnapshot.disclaimer} 部分來源失敗時，系統只顯示最近一次成功的真實快取資料，並標示 STALE。`;
...
async function fetchLiveSnapshot() {
  ...
  const snapshot = await hydrateFailedRowsWithStale(buildSnapshot(fx, rows));
