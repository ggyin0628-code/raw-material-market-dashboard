...
function buildComponent(definition = {}) {
  ...
  if (!change4WeekPct && change4WeekPct !== 0) explanation.push(`${definition.label} 沒有適合其資料頻率的 4 週對照觀測。`);
  if (!change12WeekPct && change12WeekPct !== 0) explanation.push(`${definition.label} 沒有適合其資料頻率的 12 週對照觀測。`);
...
function buildMachiningReference(input = {}) {
  ...
  const reference = {
    ...
    trend: compositePressureScore === null ? null : trendFromChange(selectedCompositeComparisonWindow?.changePct ?? compositeChange12Week ?? compositeChange4Week),
    ...
    publicPriceReferences: input.publicPriceReferences || getPublicPriceReferences({ process: "CNC" }),
    derivedMarketReference: {
      ...
      normalization: "日資料使用 4／12 週；月資料使用 1／3／12 個月；年資料使用 1／3 年；結構性資料不產生動能分數。壓力分數 = clamp(50 + 5 × 適頻率比較變化百分點, 0, 100)，可用構面依實際權重重新正規化。",
  ...
  if (errors.length || priceErrors.length) throw new Error(`machining reference contract invalid: ${[...errors, ...priceErrors].join("; ")}`);
  return reference;
}
