const STATUS_VALUES = Object.freeze(["LIVE", "FALLBACK", "STALE", "NO_DATA", "API_ERROR"]);
const PRESSURE_LEVELS = Object.freeze(["LOW", "NORMAL", "ELEVATED", "HIGH"]);
const TREND_VALUES = Object.freeze(["FALLING", "STABLE", "RISING"]);
const PUBLIC_MARKET_DISCLAIMER = "公開市場參考；非供應商報價；非公司目標價格。此頁僅反映可取得的外部公開指標與其透明推導，不代表台灣任何供應商的實際加工價格、每小時費率或報價。";
...
const TOP_LEVEL_KEYS = new Set([
  "referenceDate", "region", "machiningType", "materialFamily", "materialPressure", "energyPressure",
  "laborPressure", "fxPressure", "manufacturingPricePressure", "machineCapitalPressure",
  "compositePressureScore", "pressureLevel", "trend", "confidence", "dataQuality", "sourceProvenance",
  "explanation", "disclaimer", "observedPublicData", "derivedMarketReference", "engineeringEstimate",
]);
...
function normalizeProvenance(source = {}) {
  return {
    ...
    lastObservationDate: normalizeDate(source.lastObservationDate),
...
function validateMachiningReference(reference) {
  const errors = [];
  ...
  for (const key of ["referenceDate", "region", "machiningType", "materialFamily", "pressureLevel", "trend", "dataQuality", "disclaimer"]) {
