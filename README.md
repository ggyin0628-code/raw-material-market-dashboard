...
`production:bootstrap -- --period 3y` 保留完整三年期間；Postgres snapshot write path 使用 `POSTGRES_UPSERT_BATCH_SIZE=250` default（安全上限 500），每批單一 transaction／parameterized multi-row upsert，並以 bounded history concurrency 3（上限 4）降低 public-history fetch wall time。已提交 batch 可在 workflow interruption 後保留，rerun 不需 destructive reset。

`STORAGE_PROVIDER=filesystem` 的 production mode 仍要求 owner-approved absolute `PRODUCTION_STORAGE_ROOT`；未配置時回 `STORAGE_CONFIGURATION_REQUIRED`。`STORAGE_PROVIDER=postgres` 未配置 `DATABASE_URL` 時回 `DATABASE_URL_REQUIRED`。Production weekly 先產生 JSON／HTML／XLSX、評估 `SEND_OK`／`SEND_WITH_WARNINGS`／`SEND_BLOCKED`，再依 dry-run → `MAIL_TEST_MODE=1`／`MAIL_TEST_TO` → personal Microsoft Graph delegated OAuth2 的 staged workflow 進行 delivery。Production 使用 `MAIL_PROVIDER=outlook_graph`、`MICROSOFT_CLIENT_ID`、`MICROSOFT_REFRESH_TOKEN`、`MICROSOFT_TENANT=consumers` 與 delegated `Mail.Send`；Gmail SMTP 僅保留為明確 `smtp` compatibility provider。`npm run microsoft:oauth` 是不寄信的一次性 device-code helper，token 只能輸出至 repository 外的 mode-600 temporary file。GitHub Actions workflows 位於 [`.github/workflows/market-bootstrap.yml`](.github/workflows/market-bootstrap.yml)、[`.github/workflows/market-daily.yml`](.github/workflows/market-daily.yml) 與 [`.github/workflows/market-weekly.yml`](.github/workflows/market-weekly.yml)。Bootstrap 為 manual-only；daily／weekly 的 schedule event 只有在 repository variable `PRODUCTION_SCHEDULES_ENABLED=1` 時執行，manual dispatch 永遠可用。Weekly production CLI 只輸出 concise safe summary，不 dump full report/history。Render Free 僅作 optional dashboard hosting，不承擔排程 mail，也不提供 durable local filesystem。完成週預設以 `Asia/Taipei` 的前一個 Monday–Sunday ISO week 計算。完整模型、品質、儲存、Graph OAuth、mail 與 scheduler 說明見 [`docs/ZERO_COST_RUNTIME.md`](docs/ZERO_COST_RUNTIME.md)、[`docs/POSTGRES_STORAGE.md`](docs/POSTGRES_STORAGE.md)、[`docs/GITHUB_ACTIONS_OPERATIONS.md`](docs/GITHUB_ACTIONS_OPERATIONS.md)、[`docs/WEEKLY_MARKET_INTELLIGENCE.md`](docs/WEEKLY_MARKET_INTELLIGENCE.md)、[`docs/WEEKLY_REPORT_CONTRACT.md`](docs/WEEKLY_REPORT_CONTRACT.md)、[`docs/PRODUCTION_ACTIVATION.md`](docs/PRODUCTION_ACTIVATION.md)、[`docs/EMAIL_DELIVERY.md`](docs/EMAIL_DELIVERY.md)、[`docs/OPERATIONS_RUNBOOK.md`](docs/OPERATIONS_RUNBOOK.md) 與 [`docs/SCHEDULER_RUNBOOK.md`](docs/SCHEDULER_RUNBOOK.md)。
...
`npm test` 會 mock 外部 fetch，涵蓋材料與單位契約、conversion factor、malformed quote、日期排序與去重、bounded retry、訊號門檻、歷史計算、FX nearest-prior、fresh／stale cache、`LIVE` canonicalization、fallback、total failure、無效 symbol／period、timeout、健康檢查、兩種歷史 API 與兩種 XLSX export，以及 production storage gate、quality gate、bootstrap、SMTP compatibility、Microsoft Graph OAuth／sendMail、token/error redaction、attachment serialization、TEST mode isolation、no-fallback、concise CLI summary 與 safe weekly health。最新 deterministic suite 為 **53 passed／0 failed**，測試不依賴 Yahoo、Microsoft Graph 或 Gmail 當下可用性。
