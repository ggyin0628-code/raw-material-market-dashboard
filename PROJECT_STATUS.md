...
The existing deterministic migration test runs `migratePostgres` twice against the synthetic Postgres pool and passes both runs with the same statement count and `DATABASE_MIGRATED` state, confirming repeat-safe migration semantics. The full suite remains the required pre-push gate; the branch will record its final test count and commit SHA after the workflow contract test and all requested checks pass.
