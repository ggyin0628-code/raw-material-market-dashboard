...
## Phase 2B — Safe migration-only workflow

Phase 2B hardening remains isolated on `feat/machining-data-source-hardening-v1`; no main promotion is authorized by this task. The dedicated `.github/workflows/market-db-migrate.yml` is the only approved path for applying the new `machining_public_observations` schema. It is `workflow_dispatch` only, has no schedule, uses `permissions: contents: read`, Node 20, `DATABASE_URL`, `NODE_ENV=production`, `STORAGE_PROVIDER=postgres`, `DATABASE_SSL=true`, and `REQUIRE_DURABLE_STORAGE=1`, then runs `npm ci`, `npm run check`, `npm test`, `npm run db:migrate`, and a strict `DATABASE_MIGRATED` verification.

This workflow intentionally does not invoke `production:bootstrap`, `production:daily`, `production:weekly`, any backfill, mail, Gmail, or market collection. It is safe to dispatch repeatedly because the migration uses idempotent `CREATE TABLE IF NOT EXISTS` and `CREATE INDEX IF NOT EXISTS` statements. The repository's deterministic Postgres migration test executes the migration twice and confirms the same statement count and successful state. The workflow was not executed during this handoff, and no production deployment or secret/configuration change was performed.

Validated workflow commit: `1aeee5fff08e8562ebef4af016cbb16d25740aaf`. Full regression after the workflow contract was added: **70 passed / 0 failed**; `npm ci`, `npm run check`, `npm test`, `npm run build`, `npm audit --omit=dev`, and `git diff --check` all passed. A documentation-only follow-up may update only this handoff traceability; no workflow execution or main promotion is part of this task.
